...
| `only_active_crons` | query | no | `boolean \| null` | When true, returns only chats with an active cron schedule |  |
...
paths:
  "/ai_chats":
    get:
      ...
      description: Returns a paginated list of AI chat threads for the current authenticated
        user.
      parameters:
      - name: after
        in: query
