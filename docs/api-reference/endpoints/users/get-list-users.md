...
| `query` | query | no | `string \| null` | Search term to filter by name or username. |  |
...
- data
- page_info
description: The connection type for PublicProfileUser.
```
...
paths:
  "/users":
    get:
      ...
      summary: List users
      description: Search for users by name or username, ranked by social proximity
