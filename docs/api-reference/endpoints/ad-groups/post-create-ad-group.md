# POST /ad_groups — Create ad group
...
- **Method:** `POST`
- **Path:** `/ad_groups`
- **Operation ID:** `createAdGroup`
- **Tags:** `Ad groups`
- **Required bearer scopes:** `ad_campaign:create`, `ad_campaign:basic:read`
...
properties:
  ...
  campaign_id:
    type: string
    description: The ad campaign to create this ad group within.
  config:
    type:
    - object
    - 'null'
  ...
  status:
    ...
    - type: 'null'
    description: 'Initial status of the ad group (default: active).'
required:
- campaign_id
description: Parameters for CreateAdGroupV2
```
...
const adGroup = await client.adGroups.create({ campaign_id: 'campaign_id' });
...
ad_group = whop.ad_groups.create(campaign_id: "campaign_id")
