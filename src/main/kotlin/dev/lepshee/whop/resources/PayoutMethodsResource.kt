...
/** Operations for payout methods. */
class PayoutMethodsResource internal constructor(
    private val http: WhopHttpExecutor,
) {
    suspend fun list(
        request: dev.lepshee.whop.resources.ListPayoutMethodsRequest,
        options: WhopRequestOptions = WhopRequestOptions(),
    ): WhopPage<JsonObject> = http.get("payout_methods", request.toQueryParameters(), options)

    suspend fun retrieve(
        payoutMethodId: String,
        options: WhopRequestOptions = WhopRequestOptions(),
    ): JsonObject {
        val id = pathParameter(payoutMethodId, "Payout method ID")
        return http.get("payout_methods/$id", options = options)
    }
...
data class ListPayoutMethodsRequest(
