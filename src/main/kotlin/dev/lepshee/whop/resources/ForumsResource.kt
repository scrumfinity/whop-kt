...
import dev.lepshee.whop.models.forums.Forum
import dev.lepshee.whop.models.forums.ForumListItem
import dev.lepshee.whop.models.forums.UpdateForumRequest
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.flow

/** Operations for forums attached to experiences. */
class ForumsResource internal constructor(
    private val http: WhopHttpExecutor,
) {
    suspend fun list(
        request: dev.lepshee.whop.resources.ListForumsRequest,
        options: WhopRequestOptions = WhopRequestOptions(),
    ): WhopPage<dev.lepshee.whop.models.forums.ForumListItem> = http.get("forums", request.toQueryParameters(), options)

    suspend fun retrieve(
        forumId: String,
        options: WhopRequestOptions = WhopRequestOptions(),
    ): dev.lepshee.whop.models.forums.Forum {
        val id = pathParameter(forumId, "Forum ID")
        return http.get("forums/$id", options = options)
    }

    suspend fun update(
        forumId: String,
        request: dev.lepshee.whop.models.forums.UpdateForumRequest =
            dev.lepshee.whop.models.forums
                .UpdateForumRequest(),
        options: WhopRequestOptions = WhopRequestOptions(),
    ): dev.lepshee.whop.models.forums.Forum {
        val id = pathParameter(forumId, "Forum ID")
        return http.patch("forums/$id", request, options)
    }

    fun listAutoPaging(
        request: dev.lepshee.whop.resources.ListForumsRequest,
        options: WhopRequestOptions = WhopRequestOptions(),
    ): Flow<dev.lepshee.whop.models.forums.ForumListItem> =
        flow {
